<div class="card shadow-none border mb-4">
    <div class="card-body">
        ...
        <div class="card shadow-none border mb-4 financing-table" id="invoiceFinancingTable"
            style="display: {{ $jenis_pembiayaan === 'Invoice Financing' ? 'block' : 'none' }};">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($invoice_financing_data as $index => $invoice)
                            <tr>
                                ...
                                <td><a href="#" class="text-primary">{{ $invoice['dokumen_invoice'] }}</a></td>
        ...
        <div class="card shadow-none border mb-4 financing-table" id="poFinancingTable"
            style="display: {{ $jenis_pembiayaan === 'PO Financing' ? 'block' : 'none' }};">
            <div class="card-header">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($po_financing_data as $index => $po)
                            <tr>
                                ...
                                <td><a href="#" class="text-primary">{{ $po['dokumen_kontrak'] }}</a></td>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($installment_data as $index => $inst)
                            <tr>
                                ...
                                <td>Rp. {{ number_format((int) $inst['nilai_invoice'], 0, ',', '.') }}</td>
                                <td>{{ \Carbon\Carbon::parse($inst['invoice_date'])->format('d F Y') }}</td>
        ...
        <div class="card shadow-none border mb-4 financing-table" id="factoringTable"
            style="display: {{ $jenis_pembiayaan === 'Factoring' ? 'block' : 'none' }};">
            <div class="card-header">
                <h5 class="card-title mb-0">Kontrak Penjamin</h5>
            </div>
            <div class="table-responsive text-nowrap">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($factoring_data as $index => $fact)
                            <tr>
                                ...
                                <td>Rp. {{ number_format((int) $fact['nilai_invoice'], 0, ',', '.') }}</td>
                                <td>Rp. {{ number_format((int) $fact['nilai_pinjaman'], 0, ',', '.') }}</td>
                                <td>Rp. {{ number_format((int) $fact['nilai_bagi_hasil'], 0, ',', '.') }}</td>
                                <td>{{ \Carbon\Carbon::parse($fact['invoice_date'])->format('d F Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($fact['due_date'])->format('d F Y') }}</td>
                                <td><a href="#" class="text-primary">{{ $fact['dokumen_invoice'] }}</a></td>
        ...
        <button type="button" class="btn btn-outline-primary wave-effect" wire:click="openModal">
