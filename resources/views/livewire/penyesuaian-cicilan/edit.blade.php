<div class="container-xxl flex-grow-1 container-p-y" wire:ignore.self>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1">{{ $pageTitle ?? 'Penyesuaian Cicilan' }}</h4>
            <p class="text-muted">
                {{ $pageSubtitle ?? 'Kelola penyesuaian cicilan berdasarkan pengajuan yang telah disetujui' }}
            </p>
        </div>
        <div class="d-flex gap-2">
            @if(isset($program) && is_null($program->kontrak_generated_at))
                @can('penyesuaian_cicilan.generate_kontrak')
                    <a href="{{ route('penyesuaian-cicilan.generate-kontrak', $program->id_penyesuaian_cicilan) }}"
    ...
    <div class="card">
        <div class="card-body">
            <form wire:submit.prevent="simpan">
                ...
                @can('penyesuaian_cicilan.edit_parameter')
                                            ...
                                            @foreach ($jadwal_angsuran as $index => $item)
                                                <tr class="{{ $item['is_grace_period'] ? 'table-warning' : '' }}">
                                                    ...
                                                    <td class="text-end fw-semibold">
                                                        {{ number_format($item['total_cicilan'], 0, ',', '.') }}
                                                    </td>
                                                    <td>
                                                        @if (isset($item['status']))
                                                            @if ($item['status'] === 'Lunas')
                                                                <span class="badge bg-success">{{ $item['status'] }}</span>
                                                            @elseif($item['status'] === 'Dibayar Sebagian')
                                                    ...
                                                    @if ($isEdit)
                                                        ...
                                                        <td class="text-center">
                                                            @php
                                                                $totalDikonfirmasi = \App\Models\RiwayatPembayaranCicilan::where('id_jadwal_angsuran', $item['id'])
                                                                    ->where('status', 'Dikonfirmasi')
                                                                    ->count();
                                                                $totalTertunda = \App\Models\RiwayatPembayaranCicilan::where('id_jadwal_angsuran', $item['id'])
                                                            ...
                                                            @endphp
                                                                    ...
                                                                    <td class="text-center">
                                                                        @php
                                                                            ...
                                                                            $sisaPembayaran = $item['sisa_pembayaran'] ?? ($item['total_cicilan'] - ($item['total_terbayar'] ?? 0));
                                                                        ...
                                                                        @can('penyesuaian_cicilan.upload')
                                                                            @if ($sudahLunas)
                                                                            ...
                                                                            @elseif ($sisaPembayaran > 0)
                                                                                <div>
                                                                                    ...
                                                                                    @endif
                                                                                    @if ($sisaPembayaran < $item['total_cicilan'])
                                                                        ...
                                                                        @can('penyesuaian_cicilan.konfirmasi')
                                                                            @if ($jumlahTertunda > 0)
                                                                                <button type="button"
    ...
    @include('livewire.penyesuaian-cicilan.partials.modal')
</div>
