...
@push('scripts')
    <script>
                    ...
                    setTimeout(() => {
                        ...
                        if (projects && projects.length > 0) {
                            projects.forEach(project => {
                                $projectSelect.append(new Option(project.nama_project, project
                                    .id_project, false, false));
                            });
        ...
        $(document).ready(function() {
            ...
            $('#id_pengajuan_investasi_finlog').on('change', function() {
                ...
                setTimeout(async () => {
                    ...
                    if (projects && projects.length > 0) {
                        projects.forEach(project => {
                            $projectSelect.append(new Option(project.nama_project,
                                project.id_project, false, false));
                        });
                        $projectSelect.prop('disabled', false);
                    } else {
                        $projectSelect.prop('disabled', true);
            ...
            []);
                ...
                if (projects && projects.length > 0) {
                    projects.forEach(project => {
                        $projectSelect.append(new Option(project.nama_project, project.id_project,
                            false, false));
                    });
                    $projectSelect.prop('disabled', false);
                } else {
        ...
        $('#btnConfirmDelete').on('click', function(e) {
            ...
            $.ajax({
                url: '{{ route('sfinlog.penyaluran-deposito-sfinlog.destroy', ':id') }}'.replace(':id',
                    currentIdForDelete),
                type: 'DELETE',
                headers: {
        ...
        $('#formUploadBukti').on('submit', function(e) {
            ...
            $.ajax({
                url: '{{ route('sfinlog.penyaluran-deposito-sfinlog.upload-bukti', ':id') }}'.replace(':id',
                    currentIdForUpload),
                type: 'POST',
                data: formData,
                processData: false,
