                ...
                <div class="modal-body">
                    <form id="formTambahPengajuanInvestasi" novalidate>
                        ...
                        <div class="row">
                                    ...
                                    <select id="id_cells_project" name="id_cells_project" class="form-select select2"
                                        ...
                                        @foreach ($projects as $project)
                                            <option value="{{ $project->id_cells_project }}">
                                                {{ $project->nama_cells_bisnis }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="persentase_bagi_hasil" class="form-label">Persentase Bagi Hasil (%)/Tahun
                                    <span class="text-danger">*</span></label>
                                <div wire:ignore>
                                    <select id="persentase_bagi_hasil" name="persentase_bagi_hasil"
                                        class="form-select select2" data-placeholder="Pilih Persentase" required>
                                        <option value=""></option>
                            ...
                            <div class="col-lg-12 mb-3">
                                <label for="nominal_bagi_hasil_yang_didapat" class="form-label">Nominal Bagi Hasil Yang
                                    Didapat
                                    <span class="text-danger">*</span></label>
                                <input type="text" class="form-control input-rupiah non-editable"
                                    id="nominal_bagi_hasil_yang_didapat" placeholder="Rp 0" required disabled
                                    readonly>
                            </div>
                        </div>
                    </form>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btnSimpanPengajuanInvestasi').click(function() {
                ...
                $.ajax({
                    url: editId ? `/sfinlog/pengajuan-investasi/${editId}` :
                    ...
                    success: (res) => {
                        if (!res.error) {
                            ...
                            alert('success', res.message || (editId ?
                                'Data berhasil diperbarui' :
                                'Pengajuan investasi berhasil dibuat dan disubmit!'));
                        } else {
                    ...
                    $.ajax({
                        ...
                        success: (res) => {
                            if (!res.error) {
                                $modal.modal('hide');
                                Livewire.dispatch(
                                    'refreshPengajuanInvestasiFinlogTable');
            ...
            // Re-Submit function (for rejected pengajuan at step 1)
            $(document).on('click', '.investor-resubmit-btn', function() {
                const id = $(this).data('id');

                sweetAlertConfirm({
                    title: 'Konfirmasi Re-Submit',
                    text: 'Apakah Anda yakin ingin re-submit pengajuan ini? Setelah re-submit, data akan kembali divalidasi oleh Finance SKI.',
                    icon: 'question',
                    confirmButtonText: 'Ya, Re-Submit',
                    cancelButtonText: 'Batal',
                }, () => {
                    $.ajax({
                        url: `/sfinlog/pengajuan-investasi/${id}/approval`,
                        method: 'POST',
                        data: {
                            _token: CSRF,
                            status: 'Submit Pengajuan'
                        },
                        success: (res) => {
                            if (!res.error) {
                                Livewire.dispatch(
                                    'refreshPengajuanInvestasiFinlogTable');
                                alert('success', res.message ||
                                    'Pengajuan berhasil di-submit ulang!');
                            } else {
                                alert('error', res.message);
                            }
                        },
