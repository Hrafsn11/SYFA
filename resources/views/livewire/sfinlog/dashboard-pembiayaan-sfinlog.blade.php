<div>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-0">Dashboard Pembiayaan Sfinlog</h4>
        </div>
    </div>
        ...
        <div class="col-12 col-md-6 col-xl-3">
            <div class="card h-100">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            <h6 class="text-muted mb-2">Total Yang Belum Dibayarkan Piutang</h6>
                            <h4 class="mb-2 fw-bold">Rp
        ...
        <div class="col-12 col-xl-6">
            <div class="card h-100">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Total Disburse Pokok dan Bagi Hasil</h5>
                    <div wire:ignore style="width: 150px;">
                        <select id="filterBulanDisbursement" class="form-select select2" data-placeholder="Pilih Bulan">
                            <option value=""></option>
                            @foreach ($bulanNama as $value => $label)
                                <option value="{{ $value }}"
                    ...
                    <div wire:ignore style="width: 150px;">
                        <select id="filterTahunPiutang" class="form-select select2" data-placeholder="Pilih Tahun">
                            @foreach (range(date('Y') - 5, date('Y') + 1) as $t)
                                <option value="{{ $t }}" {{ $tahunPiutang == $t ? 'selected' : '' }}>
                                    {{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <div class="row g-4 mb-4">
        <div class="col-12 col-xl-6">
            <div class="card h-100">
                <div class="card-header">
                    <h5 class="card-title mb-3">Total AR Berdasarkan Kriteria Keterlambatan</h5>
                    <div class="row g-2">
                        <div class="col-6">
                            <div wire:ignore>
                        ...
                        <div class="col-6">
                            <div wire:ignore>
                                <select id="filterTahunTable" class="form-select select2"
                                    ...
                                    @foreach (range(date('Y') - 5, date('Y') + 1) as $t)
                                        <option value="{{ $t }}"
                                            {{ $tahunTable == $t ? 'selected' : '' }}>{{ $t }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        <div class="col-12 col-xl-6">
            <div class="card h-100">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Perbandingan AR dan Utang Pengembalian</h5>
                    <div class="d-flex gap-2">
                        <div wire:ignore style="width: 120px;">
                            <select id="filterBulanComparison1" class="form-select select2"
                                ...
                                <option value=""></option>
                                @foreach ($bulanNama as $value => $label)
                                    <option value="{{ $value }}" {{ $bulan1 == $value ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div wire:ignore style="width: 120px;">
                            <select id="filterBulanComparison2" class="form-select select2"
                                ...
                                <option value=""></option>
                                @foreach ($bulanNama as $value => $label)
                                    <option value="{{ $value }}" {{ $bulan2 == $value ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row g-3 mt-3">
                        <div class="col-6">
                            <div class="p-3 rounded-3" style="background-color: #f0f7ff;">
                                <small class="text-muted d-block mb-1">Selisih AR</small>
                                ...
                                <small class="text-muted">
                                    ...
                                    @else
                                        <i class="ti ti-minus text-secondary"></i> Tidak berubah
                                    @endif
                                </small>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="p-3 rounded-3" style="background-color: #fff8f0;">
                                <small class="text-muted d-block mb-1">Selisih Utang</small>
                                <h5 class="mb-0 fw-bold">Rp
...
@push('scripts')
    <script>
        (function() {
            ...
            function getBarOptions(series, categories, colors = ['#71dd37', '#ffab00']) {
                return {
                    series: series,
                    chart: {
                        type: 'bar',
            ...
            function createChart(elementId, series, categories, colors = ['#71dd37', '#ffab00']) {
            ...
            function renderCharts() {
                ...
                if (charts.disbursement) {
                    updateChart(charts.disbursement, disbursementSeries, data.disbursement.categories || []);
                } else {
                    charts.disbursement = createChart('chartDisbursement', disbursementSeries, data.disbursement
                        .categories || []);
                }
                ...
                if (charts.pembayaran) {
                    updateChart(charts.pembayaran, pembayaranSeries, data.pembayaran.categories || []);
                } else {
                    charts.pembayaran = createChart('chartPembayaran', pembayaranSeries, data.pembayaran.categories ||
                    []);
                }
                ...
                if (charts.sisa) {
                    updateChart(charts.sisa, sisaSeries, data.sisa.categories || []);
                } else {
                    charts.sisa = createChart('chartSisaBelumTerbayar', sisaSeries, data.sisa.categories || []);
                }
                ...
                if (charts.piutang) {
                    updateChart(charts.piutang, piutangSeries, data.piutang.categories || []);
                } else {
                    charts.piutang = createChart('chartPembayaranPiutang', piutangSeries, data.piutang.categories ||
                []);
                }

                const comparisonSeries = [{
                        name: 'AR',
                        data: data.comparison.ar || []
                    },
                    {
                        name: 'Utang Pengembalian',
                        data: data.comparison.utang_pengembalian_deposito || []
                    }
                ];
                if (charts.comparison) {
                    updateChart(charts.comparison, comparisonSeries, data.comparison.categories || []);
                } else {
                    charts.comparison = createChart('chartComparison', comparisonSeries, data.comparison.categories ||
                    []);
                }
            }
