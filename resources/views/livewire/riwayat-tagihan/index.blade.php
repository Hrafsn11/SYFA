                ...
                <div class="modal-content">
                    ...
                    <form id="editDebiturPiutangForm">
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="edit_persentase_bunga" class="form-label">% Bunga</label>
                                <div class="input-group">
                                    <input type="number" step="0.01" class="form-control"
                                        id="edit_persentase_bunga" name="persentase_bunga" required>
                                    <span class="input-group-text">%</span>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label for="edit_kurang_bayar_bunga" class="form-label">Total Kurang Bayar Bagi
                                    Hasil</label>
                                <input type="text" class="form-control money-format" id="edit_kurang_bayar_bunga"
                                    name="kurang_bayar_bunga" required>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(e) {
                const btn = e.target.closest('.edit-riwayat-tagihan-btn');
                if (!btn) return;
                ...
                document.getElementById('edit_nilai_dicairkan').value = formatNumber(data.nilai_dicairkan ||
                    0);
                document.getElementById('edit_persentase_bunga').value = data.persentase_bunga ||
                    0;
                document.getElementById('edit_kurang_bayar_bunga').value = formatNumber(data
                    .kurang_bayar_bunga ||
                    0);
            ...
            // Auto format money input for kurang_bayar_bunga
            document.getElementById('edit_kurang_bayar_bunga').addEventListener('input', function(e) {
                let value = e.target.value.replace(/\D/g, '');
                e.target.value = formatNumber(value);
            });
            ...
            document.getElementById('editDebiturPiutangForm').addEventListener('submit', function(e) {
                ...
                const formData = {
                    ...
                    persentase_bunga: parseFloat(document.getElementById(
                        'edit_persentase_bunga').value),
                    kurang_bayar_bunga: parseFormattedNumber(document.getElementById(
                        'edit_kurang_bayar_bunga').value),
                };

                fetch('{{ route('riwayat-tagihan.update') }}', {
                        method: 'POST',
                        headers: {
