...
@push('scripts')
    <script>
        ...
        function editDataDirect(button) {
            ...
            // FIX: Ensure nominal is a clean number (remove any existing formatting first)
            const cleanNominal = String(data.nominal_yang_disalurkan).replace(/[^0-9]/g, '');

            // Set hidden field with raw value
            $('#nominal_raw').val(cleanNominal);

            // Format untuk display
            $('#nominal_yang_disalurkan').val(formatRupiah(cleanNominal));

            // Update Livewire properties dengan raw value
            @this.set('id', data.id);
            @this.set('id_pengajuan_investasi', data.id_pengajuan_investasi);
            @this.set('id_debitur', data.id_debitur);
            @this.set('nominal_yang_disalurkan', cleanNominal); // Use clean number!
        ...
        $('#formUploadBukti').on('submit', function(e) {
            ...
            $.ajax({
                url: '{{ route('penyaluran-deposito.upload-bukti', ':id') }}'.replace(':id',
                    currentIdForUpload),
                type: 'POST',
                data: formData,
                processData: false,
