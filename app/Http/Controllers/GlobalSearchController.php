...
class GlobalSearchController extends Controller
{
    ...
    private function searchPages(string $q): array
    {
        $pages = [
            ...
            ['title' => 'Report Penyaluran Dana Investasi', 'route' => 'report-penyaluran-dana-investasi.index', 'keywords' => ['report', 'penyaluran', 'investasi']],
            ['title' => 'Penyaluran Deposito', 'route' => 'penyaluran-deposito.index', 'keywords' => ['penyaluran', 'deposito', 'deposit']],
    ...
    private function getIconForPage(string $title): string
    {
        ...
        if (str_contains($titleLower, 'dashboard')) return 'ti-smart-home';
        if (str_contains($titleLower, 'peminjaman') || str_contains($titleLower, 'pinjaman')) return 'ti-briefcase';
        if (str_contains($titleLower, 'investasi')) return 'ti-coin';
        if (str_contains($titleLower, 'restrukturisasi')) return 'ti-file-text';
        if (str_contains($titleLower, 'pengembalian')) return 'ti-wallet';
        if (str_contains($titleLower, 'ar ')) return 'ti-chart-line';
        if (str_contains($titleLower, 'debitur') || str_contains($titleLower, 'investor')) return 'ti-users';
        if (str_contains($titleLower, 'master') || str_contains($titleLower, 'kol')) return 'ti-database';
        if (str_contains($titleLower, 'config')) return 'ti-settings';
        if (str_contains($titleLower, 'users') || str_contains($titleLower, 'roles') || str_contains($titleLower, 'permissions')) return 'ti-lock';
