...
use App\Models\{
    EvaluasiAnalisaCicilan,
    EvaluasiKelayakanDebitur,
    EvaluasiKelengkapanDokumen,
    EvaluasiPengajuanCicilan,
    HistoryStatusPengajuanCicilan,
    PengajuanPeminjaman,
    PengajuanCicilan,
    PersetujuanKomiteCicilan
};
...
class EvaluasiCicilanController extends Controller
{
    ...
    public function decision(Request $request, $id): JsonResponse
    {
        try {
            ...
            $historyRecord = HistoryStatusPengajuanCicilan::create($history);
    ...
    private function createOrUpdateEvaluasi(string $id, array $data): EvaluasiPengajuanCicilan
    {
        $evaluasi = EvaluasiPengajuanCicilan::firstOrCreate(
            ['id_pengajuan_cicilan' => $id]
        );
    ...
    private function syncSections(EvaluasiPengajuanCicilan $evaluasi, array $sections, Request $request): void
    {
        $evaluasiId = $evaluasi->id_evaluasi_cicilan;
    ...
    private function syncKelengkapanDokumen($evaluasiId, array $data): void
    {
        ...
        EvaluasiKelengkapanDokumen::where('id_evaluasi_cicilan', $evaluasiId)->delete();

        foreach ($data as $row) {
            EvaluasiKelengkapanDokumen::create([
                'id_evaluasi_cicilan' => $evaluasiId,
    ...
    private function syncKelayakanDebitur($evaluasiId, array $data): void
    {
        ...
        EvaluasiKelayakanDebitur::where('id_evaluasi_cicilan', $evaluasiId)->delete();
    ...
    private function syncAnalisaRestrukturisasi($evaluasiId, array $data): void
    {
        ...
        EvaluasiAnalisaCicilan::where('id_evaluasi_cicilan', $evaluasiId)->delete();
    ...
    private function syncPersetujuanKomite($evaluasiId, Request $request): void
    {
        ...
        PersetujuanKomiteCicilan::where('id_evaluasi_cicilan', $evaluasiId)->delete();
    ...
    private function buildHistoryData(PengajuanCicilan $pengajuan, array $validated): array
    {
        return [
            'id_pengajuan_cicilan' => $pengajuan->id_pengajuan_cicilan,
    ...
    private function handleApproval(PengajuanCicilan $pengajuan, int $step, array &$history): void
    {
    ...
    private function handleRejection(PengajuanCicilan $pengajuan, int $step, array &$history): void
    {
        $rejectionHandlers = [
    ...
    private function rejectStep2(PengajuanCicilan $pengajuan): void
    {
    ...
    private function rejectStep3(PengajuanCicilan $pengajuan): void
    {
    ...
    private function rejectStep4(PengajuanCicilan $pengajuan): void
    {
    ...
    private function rejectDefault(PengajuanCicilan $pengajuan, int $step): void
    {
