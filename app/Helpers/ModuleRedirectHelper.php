...
class ModuleRedirectHelper
{
    ...
    public static function redirectToFirstAccessible(string $module = 'sfinance')
    {
        ...
        // Define route priority for each module
        $routePriorities = self::getRoutePriorities($module);
        ...
        // If no accessible route found, redirect to home services with error
        return redirect()->route('home.services')
            ->with('error', 'Anda tidak memiliki akses ke modul ini.');
    }

    private static function getRoutePriorities(string $module): array
    {
        $routes = [
            'sfinance' => [
