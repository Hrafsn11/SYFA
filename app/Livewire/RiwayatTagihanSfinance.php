...
class RiwayatTagihanSfinance extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Bunga Oleh Debitur', 'total_bunga')
                ->label(fn($row) => 'Rp ' . number_format($row->total_bunga ?? 0, 0, ',', '.'))
                ->sortable(),
            ...
            Column::make('Total Sisa Pokok + Bunga')->label(function ($row) {
                $sisaPokok = $row->sisa_pokok ?? 0;
                $sisaBagiHasil = $row->kurang_bayar_bunga ?? 0;
                return 'Rp ' . number_format($sisaPokok + $sisaBagiHasil, 0, ',', '.');
            }),

            Column::make('Total Kurang Bayar Bunga', 'kurang_bayar_bunga')
                ->label(fn($row) => 'Rp ' . number_format($row->kurang_bayar_bunga ?? 0, 0, ',', '.'))
                ->sortable(),
            ...
            Column::make('% Bunga', 'persentase_bunga')
                ->label(fn($row) => ($row->persentase_bunga ?? 0) . '%')
                ->sortable(),
            ...
            Column::make('Total Bunga Saat Ini', 'total_bunga_saat_ini')
                ->label(function ($row) {
                    // Prioritas: total_bunga_saat_ini dari database, fallback ke total_bunga
                    $total = $row->total_bunga_saat_ini ?? $row->total_bunga ?? 0;
            ...
            Column::make('Bunga/Bulan')->label(function ($row) {
                $bagiHasil = $row->total_bunga ?? 0;
            ...
            Column::make('Subtotal Sisa Pokok + Bunga')
                ->label(function ($row) {
                    ...
                    $sisaBagiHasil = $row->kurang_bayar_bunga ?? $row->total_bunga ?? 0;
