...
use App\Models\EvaluasiPengajuanRestrukturisasi;
...
class ValidasiPengajuanRestrukturisasi extends Component
{
    ...
    {
        $this->pengajuan = PengajuanRestrukturisasi::with(['debitur', 'peminjaman'])->findOrFail($this->pengajuanId);
        $this->debitur = $this->pengajuan->debitur;
        
        // Load existing committee data
        $evaluasi = EvaluasiPengajuanRestrukturisasi::where('id_pengajuan_restrukturisasi', $this->pengajuanId)->first();
        if ($evaluasi) {
            $persetujuans = PersetujuanKomiteRestrukturisasi::where('id_evaluasi_restrukturisasi', $evaluasi->id_evaluasi_restrukturisasi)
                ->orderBy('urutan')
                ->get();
        ...
        // Load history
        $this->allHistory = HistoryStatusPengajuanRestrukturisasi::where('id_pengajuan_restrukturisasi', $this->pengajuanId)
    ...
    public function saveEvaluasiFromJS($payload)
    {
        ...
        // Permission check
        if (!Gate::allows('pengajuan_restrukturisasi.validasi_dokumen')) {
        ...
        try {
            ...
            $evaluasi = EvaluasiPengajuanRestrukturisasi::firstOrCreate([
                'id_pengajuan_restrukturisasi' => $this->pengajuanId,
            ]);
            ...
            Log::info('Evaluasi header saved', ['id' => $evaluasi->id_evaluasi_restrukturisasi]);

            // ✅ Save Section A: Kelengkapan Dokumen
            if (!empty($payload['kelengkapan']) && is_array($payload['kelengkapan'])) {
                EvaluasiKelengkapanDokumen::where('id_evaluasi_restrukturisasi', $evaluasi->id_evaluasi_restrukturisasi)->delete();
                
                foreach ($payload['kelengkapan'] as $idx => $row) {
                    EvaluasiKelengkapanDokumen::create([
                        'id_evaluasi_restrukturisasi' => $evaluasi->id_evaluasi_restrukturisasi,
            ...
            if (!empty($payload['kelayakan']) && is_array($payload['kelayakan'])) {
                EvaluasiKelayakanDebitur::where('id_evaluasi_restrukturisasi', $evaluasi->id_evaluasi_restrukturisasi)->delete();
            ...
            if (!empty($payload['analisa']) && is_array($payload['analisa'])) {
                EvaluasiAnalisaRestrukturisasi::where('id_evaluasi_restrukturisasi', $evaluasi->id_evaluasi_restrukturisasi)->delete();
            ...
            if (!empty($this->committee) && is_array($this->committee)) {
                ...
                PersetujuanKomiteRestrukturisasi::where('id_evaluasi_restrukturisasi', $evaluasi->id_evaluasi_restrukturisasi)->delete();
    ...
    {
        if (!Gate::allows('pengajuan_restrukturisasi.ajukan_restrukturisasi')) {
            $this->dispatch('evaluasiError', message: 'Unauthorized');
            return;
        }
        ...
        try {
            ...
            HistoryStatusPengajuanRestrukturisasi::create([
                'id_pengajuan_restrukturisasi' => $this->pengajuan->id_pengajuan_restrukturisasi,
    ...
    public function handleApprove($step)
    {
        ...
        if ($step == 2 && !Gate::allows('pengajuan_restrukturisasi.validasi_dokumen')) {
            $this->dispatch('evaluasiError', message: 'Unauthorized');
            return;
        }
        if ($step == 3 && !Gate::allows('pengajuan_restrukturisasi.persetujuan_ceo_ski')) {
            $this->dispatch('evaluasiError', message: 'Unauthorized');
            return;
        }
        if ($step == 4 && !Gate::allows('pengajuan_restrukturisasi.persetujuan_direktur')) {
            $this->dispatch('evaluasiError', message: 'Unauthorized');
            return;
        }
