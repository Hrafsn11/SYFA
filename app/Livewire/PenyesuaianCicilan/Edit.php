...
use App\Models\RiwayatPembayaranCicilan;
...
class Edit extends Create
{
    ...
    public PengajuanCicilan $PengajuanCicilan;

    public string $pageTitle = 'Aksi Penyesuaian Cicilan';
    public string $pageSubtitle = 'Perbarui parameter penyesuaian cicilan';
    ...
    {
        abort_unless(auth()->user()->can('penyesuaian_cicilan.edit'), 403, 'Unauthorized');
        ...
        $this->PengajuanCicilan = $this->program->PengajuanCicilan;
        if (in_array('Pengurangan tunggakan pokok/margin', $this->PengajuanCicilan->jenis_restrukturisasi)) {
        ...
        $this->loadApprovedRestrukturisasi($this->id_pengajuan_cicilan);
    }

    public function render()
    {
        return view('livewire.penyesuaian-cicilan.edit')->layout('layouts.app');
    }
    ...
    private function initializeFromProgram(): void
    {
        ...
        $this->id_pengajuan_cicilan = $this->program->id_pengajuan_cicilan;
    ...
    private function mapJadwalAngsuran($jadwalCollection): array
    {
        ...
        return $jadwalCollection->map(function ($item) use (&$sisaPokok) {
            ...
            $totalTertunda = $item->riwayatPembayaran()
                ->where('status', RiwayatPembayaranCicilan::STATUS_TERTUNDA)
                ->sum('nominal_bayar');
    ...
    public function simpan(): void
    {
        ...
        $khususPenguranganTunggakanPokok = in_array('Pengurangan tunggakan pokok/margin', $this->PengajuanCicilan->jenis_restrukturisasi);
        ...
        try {
            ...
            $this->showSuccess('Berhasil', 'Penyesuaian cicilan berhasil diperbarui!', route('penyesuaian-cicilan.index'));
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    private function validateInput(): void
    {
        ...
        $this->validate($validate, [
            'id_pengajuan_cicilan.required' => 'Silakan pilih pengajuan restrukturisasi.',
    ...
    {
        $this->riwayatPembayaran = RiwayatPembayaranCicilan::where('id_jadwal_angsuran', $idJadwalAngsuran)
    ...
    public function submitUploadBukti(): void
    {
        ...
        try {
            ...
            $riwayat = RiwayatPembayaranCicilan::create([
                'id_jadwal_angsuran' => $angsuran['id'],
                'nominal_bayar' => $nominalBayar,
                'bukti_pembayaran' => $path,
                'tanggal_bayar' => now(),
                'status' => RiwayatPembayaranCicilan::STATUS_TERTUNDA,
            ]);
    ...
    private function uploadBuktiPembayaran($file, int $noAngsuran): string
    {
        $filename = sprintf(
            ...
            $this->program->id_penyesuaian_cicilan,
            $noAngsuran,
            time(),
    ...
    {
        if (!auth()->user()->can('penyesuaian_cicilan.konfirmasi')) {
    ...
    public function submitKonfirmasi(): void
    {
        ...
        try {
            ...
            $riwayat = RiwayatPembayaranCicilan::find($this->selectedRiwayatId);
            if (!$riwayat)
                throw new \Exception('Data riwayat pembayaran tidak ditemukan di database.');

            // Update status riwayat pembayaran
            $riwayat->update([
                'status' => RiwayatPembayaranCicilan::STATUS_DIKONFIRMASI,
    ...
    public function tolakPembayaran(): void
    {
        ...
        try {
            ...
            $riwayat->update([
                'status' => RiwayatPembayaranCicilan::STATUS_DITOLAK,
    ...
    private function updateProgramTotalTerbayar(): void
    {
        ...
        $totalTerbayar = JadwalAngsuran::where('id_penyesuaian_cicilan', $this->program->id_penyesuaian_cicilan)
            ->sum('total_terbayar');
