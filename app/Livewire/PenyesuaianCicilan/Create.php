...
use App\Http\Requests\PenyesuaianCicilanRequest;
...
class Create extends Component
{
    ...
    public string $pageTitle = 'Form Tambah Penyesuaian Cicilan';
    public string $pageSubtitle = 'Buat penyesuaian cicilan berdasarkan pengajuan yang telah disetujui';
    public string $submitLabel = 'Simpan Penyesuaian Cicilan';

    // Form fields
    public $id_pengajuan_cicilan = '';
    ...
    protected function loadApprovedRestrukturisasi(?string $forceIncludeId = null): void
    {
        ...
        $query = PengajuanCicilan::with('debitur')
            ->where(function ($q) {
            ...
            ->whereDoesntHave('PenyesuaianCicilan');
        ...
        if ($forceIncludeId) {
            $query->orWhere('id_pengajuan_cicilan', $forceIncludeId);
        }
    ...
    public function handleComponentEvent($value, $modelName)
    {
        if (property_exists($this, $modelName)) {
            ...
            if ($modelName === 'id_pengajuan_cicilan') {
                $this->loadPengajuanData();
            }
        }
    }

    public function updatedIdPengajuanCicilan()
    {
        $this->loadPengajuanData();
    }
    ...
    {
        if ($this->id_pengajuan_cicilan) {
            $this->restrukturisasi = PengajuanCicilan::with('debitur')
                ->find($this->id_pengajuan_cicilan);
    ...
    public function updated($propertyName)
    {
        ...
        if ($propertyName === 'id_pengajuan_cicilan') {
            $this->updatedIdPengajuanCicilan();
        }
    }
    ...
    public function hitungJadwalAngsuran()
    {
        ...
        if (empty($this->id_pengajuan_cicilan)) {
            $this->addError('id_pengajuan_cicilan', 'Silakan pilih Pengajuan Restrukturisasi terlebih dahulu.');
            return;
        }
    ...
    public function simpan()
    {
        ...
        try {
            $request = new PenyesuaianCicilanRequest();
        ...
        try {
            ...
            $program = PenyesuaianCicilan::create($filled);

            if (!in_array('Pengurangan tunggakan pokok/margin', $program->PengajuanCicilan->jenis_restrukturisasi)) {
                foreach ($this->jadwal_angsuran as $item) {
                    JadwalAngsuran::create([
                        'id_penyesuaian_cicilan' => $program->id_penyesuaian_cicilan,
                        'no' => $item['no'],
            ...
            ListNotifSFinance::createPenyesuaianCicilan($program);
            DB::commit();
            ...
            $this->dispatch('swal:modal', [
                ...
                'redirect_url' => route('penyesuaian-cicilan.generate-kontrak', $program->id_penyesuaian_cicilan)
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function getCanCalculateProperty(): bool
    {
        if ($this->restrukturisasi) {
            if (!in_array('Pengurangan tunggakan pokok/margin', $this->restrukturisasi->jenis_restrukturisasi)) {
                return !empty($this->id_pengajuan_cicilan)
                    && $this->plafon_pembiayaan > 0
    ...
    public function getCanSaveProperty(): bool
    {
        return !empty($this->jadwal_angsuran)
            && $this->show_jadwal
            && !empty($this->id_pengajuan_cicilan);
    }

    public function render()
    {
        return view('livewire.penyesuaian-cicilan.create')->layout('layouts.app');
    }
}
